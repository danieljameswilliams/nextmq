...
import { ArrowRight, CheckCircle2, Clock, Database, Package, PlayCircle, Radio, RotateCcw, Zap } from 'lucide-react'
  ...
  const phases: Phase[] = [
    {
      ...
      details: (
        <div className="mt-3 p-3 bg-blue-50 dark:bg-blue-950/30 rounded-lg border border-blue-200 dark:border-blue-900">
          <p className="text-xs text-blue-800 dark:text-blue-200 font-mono mb-1">&lt;NextMQRootClientEventBridge /&gt;</p>
          <p className="text-xs text-blue-700 dark:text-blue-300">~1KB • Listening for 'nextmq' CustomEvents</p>
        </div>
      ),
    },
    {
      ...
      details: (
        <div className="mt-3 space-y-2">
          <div className="p-3 bg-purple-50 dark:bg-purple-950/30 rounded-lg border border-purple-200 dark:border-purple-900">
            <p className="text-xs font-semibold text-purple-900 dark:text-purple-100 mb-2">Event Buffer ({eventBufferCount} events)</p>
            {eventBufferCount > 0 && (
            ...
            <p className="text-xs text-purple-700 dark:text-purple-300 mt-2">Waiting for processor...</p>
          </div>
        </div>
      ),
    },
    {
      ...
      details: (
        <div className="mt-3 p-3 bg-green-50 dark:bg-green-950/30 rounded-lg border border-green-200 dark:border-green-900">
          <p className="text-xs text-green-800 dark:text-green-200 mb-1">&lt;NextMQClientProvider processor={'{processor}'} /&gt;</p>
          <p className="text-xs text-green-700 dark:text-green-300 font-mono">Dynamic import: processor chunk loads</p>
          <p className="text-xs text-green-600 dark:text-green-400 mt-1">Code-split • Only loads when needed</p>
        </div>
      ),
    },
    {
      ...
      details: (
        <div className="mt-3 flex items-center gap-3">
          <div className="flex-1 p-3 bg-purple-50 dark:bg-purple-950/30 rounded-lg border border-purple-200 dark:border-purple-900">
            <p className="text-xs font-semibold text-purple-900 dark:text-purple-100 mb-1">Event Buffer</p>
            <p className="text-xs text-purple-700 dark:text-purple-300">{eventBufferCount} events → moving</p>
          </div>
          ...
          <div className="flex-1 p-3 bg-blue-50 dark:bg-blue-950/30 rounded-lg border border-blue-200 dark:border-blue-900">
            <p className="text-xs font-semibold text-blue-900 dark:text-blue-100 mb-1">Job Queue</p>
            <p className="text-xs text-blue-700 dark:text-blue-300">{queueCount} jobs ready</p>
          </div>
        </div>
      ),
    },
    {
      ...
      details: (
        <div className="mt-3 space-y-2">
          ...
          <p className="text-xs text-gray-600 dark:text-gray-400">Only jobs with met requirements execute</p>
        </div>
      ),
    },
    {
      ...
      details: (
        <div className="mt-3 p-3 bg-green-50 dark:bg-green-950/30 rounded-lg border border-green-200 dark:border-green-900">
          <p className="text-xs text-green-800 dark:text-green-200 mb-1">Handler chunk loads dynamically</p>
          <p className="text-xs text-green-700 dark:text-green-300 font-mono">_next/static/chunks/handlers_cartAdd-[hash].js</p>
    ...
    <div className="space-y-6">
      <div className="text-center mb-6">
        ...
        <div className="flex items-center justify-center gap-2">
          <button type="button" onClick={play} className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition-colors">
            <PlayCircle className="w-4 h-4" />
            {isPlaying ? 'Playing...' : 'Play Animation'}
          </button>
          <button type="button" onClick={reset} className="inline-flex items-center gap-2 px-4 py-2 bg-gray-200 dark:bg-gray-800 hover:bg-gray-300 dark:hover:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg text-sm font-medium transition-colors">
            ...
            {phases.map((phase, index) => {
              ...
              return (
                <div key={phase.id} className="relative flex items-start gap-6">
                  {/* Icon */}
                  <div
                    className={`relative z-10 flex items-center justify-center w-16 h-16 rounded-full border-2 transition-all shrink-0 ${
                      status === 'completed'
                  ...
                  <div className="flex-1 pt-2">
                    <div className="flex items-center gap-3 mb-1 flex-wrap">
                      ...
                      <span className="text-xs text-gray-500 dark:text-gray-400 font-mono bg-gray-100 dark:bg-gray-900 px-2 py-0.5 rounded">{phase.timing}</span>
      ...
      <div className="bg-gradient-to-r from-blue-50 to-purple-50 dark:from-blue-950/30 dark:to-purple-950/30 rounded-xl p-6 border border-blue-200 dark:border-blue-900">
        ...
        <div className="grid md:grid-cols-2 gap-4">
          <div className="space-y-2">
            <h5 className="font-semibold text-sm text-gray-900 dark:text-gray-100">⚡ First Bytes Optimization</h5>
            <p className="text-sm text-gray-700 dark:text-gray-300">Event Bridge is only ~1KB and loads immediately. It starts listening for events right away, ensuring no events are lost.</p>
          </div>
          <div className="space-y-2">
            <h5 className="font-semibold text-sm text-gray-900 dark:text-gray-100">📦 Smart Buffering</h5>
            <p className="text-sm text-gray-700 dark:text-gray-300">Events dispatched before the processor loads are automatically buffered and moved to the queue when ready.</p>
          </div>
          <div className="space-y-2">
            <h5 className="font-semibold text-sm text-gray-900 dark:text-gray-100">🚀 Dynamic Loading</h5>
            <p className="text-sm text-gray-700 dark:text-gray-300">Processor and handlers load on-demand via code-splitting. Only what you need, when you need it.</p>
          </div>
          <div className="space-y-2">
            <h5 className="font-semibold text-sm text-gray-900 dark:text-gray-100">✅ Requirements Gating</h5>
            <p className="text-sm text-gray-700 dark:text-gray-300">Jobs wait until their requirements are met. Perfect for consent, authentication, or feature flags.</p>
          </div>
        </div>
      </div>
